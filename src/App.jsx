...
function Card({ children, style }) {
  return (
    <div style={{ background: "#1e1e2e", borderRadius: 12, padding: "16px 20px", ...style }}>
      {children}
    </div>
  );
}
...
function BankTab() {
  ...
  return (
    <div>
      ...
      <Card>
        <Table
          rows={data.data.slice(0, 50)}
          columns={[
            { key: "name", label: "שם" },
            { key: "ticker", label: "טיקר" },
            { key: "portfolio", label: "תיק" },
            { key: "status", label: "סטטוס", render: (r) => <Badge text={r.status || "—"} color={r.status === "Open" || r.status === "פתוח" ? "#166534" : "#4b5563"} /> },
          ...
          <Card key={p.name}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#aaa", marginTop: 4 }}>
              ...
              <span style={{ color: "#22c55e" }}>
                {p.closed > 0 ? pct((p.winners / p.closed) * 100) : "—"}
              </span>
            </div>
          </Card>
        ))}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {["all", "open", "closed"].map((f) => (
          <button key={f} onClick={() => setFilter(f)} style={{
            ...
            background: filter === f ? "#6366f1" : "#2a2a3e", color: "#fff", fontSize: 13,
          }}>
      ...
      {Object.values(bySector).map((sector) => (
        <div key={sector.name} style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {sector.stocks.map((s) => (
              <div key={s.id} style={{
                background: heatColor(s.pnlPercent ?? s.pnl),
                borderRadius: 8, padding: "10px 14px", minWidth: 90, textAlign: "center",
              }}>
  ...
  const Field = ({ label, value, onChange, placeholder }) => (
    <div style={{ marginBottom: 14 }}>
      ...
      <input
        ...
        style={{ width: "100%", background: "#2a2a3e", border: "1px solid #444", borderRadius: 8, padding: "8px 12px", color: "#fff", fontSize: 14, boxSizing: "border-box" }}
      />
    </div>
  );
...
function DailyUpdateTab() {
  ...
  useEffect(() => {
    fetchStocks({ tab: "trades", status: "Open" })
      .then(setData)
      .catch((e) => setError(e.message))
      .finally(() => setLoading(false));
  }, []);
  ...
  const totalExposure = open.reduce((s, r) => s + (r.entryPrice ?? 0), 0);
